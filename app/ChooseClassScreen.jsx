...
export default function ClassListScreen() {
  ...
  const fetchClassesFromDb = async () => {
    try {
      ...
      const fetchedClasses = res.data.data.map((cls) => ({
        ...cls,
        studentCount: cls.studentCount || 0, // ensure studentCount is always present
      }));
  ...
  // Load classes from AsyncStorage, then fetch fresh data
  const loadClasses = async () => {
    try {
      const storedData = await AsyncStorage.getItem(STORAGE_KEY);
      if (storedData) {
        const parsed = JSON.parse(storedData);
        setClasses(parsed.map((cls) => ({ ...cls, studentCount: cls.studentCount || 0 })));
      }
      await fetchClassesFromDb(); // fetch fresh data after showing cached
    } catch (err) {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => router.push(`/StudentListScreen?classId=${item._id}&schoolId=${schoolId}`)}
    >
      <LinearGradient
        ...
        <View style={styles.cardContent}>
          <View>
            ...
            <Text style={styles.studentCount}>
              👥 {item.studentCount} জন ছাত্র/ছাত্রী
            </Text>
          </View>
          <Ionicons
  ...
  if (loading) {
    return (
      <View style={[styles.container, { justifyContent: "center", alignItems: "center" }]}>
        <ActivityIndicator size="large" color="#0a47a1" />
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  className: { fontSize: 18, fontWeight: "700", color: "#003366" },
