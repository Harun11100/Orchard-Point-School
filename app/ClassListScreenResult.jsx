...
export default function ClassListScreenResult() {
  ...
  const normalizeClassData = (cls) => ({
    ...
    studentCount:
      cls.studentCount ||
      cls["studentCoun  nt"] || // handle typo in backend
      0,
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() =>
      router.push({
      pathname: "/StudentListScreenResult",
      params: {
      classId: item._id,
      schoolId,
  ...
  if (loading) {
    return (
      <View
        style={[styles.container, { justifyContent: "center", alignItems: "center",backgroundColor: "#f4f5ffff", }]}
      >
  ...
  return (
    <LinearGradient colors={["#f8faffff", "#f1f9ffff"]} style={styles.container}>
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 18, backgroundColor:"rgba(244, 255, 251, 1)" },
  title: {
    fontSize: 24,
    fontWeight: "800",
   color: "#315cb2ff",
  ...
  className: { fontSize: 18, fontWeight: "700", color: "#1558afff" },
});
